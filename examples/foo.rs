...
fn main() {
  ...
  for line in ENUMERATIONS.lines() {
    if line.is_empty() {
      continue;
      ...
      let strip_pub_const = &line["pub const ".len()..];
      let stop = strip_pub_const
        .bytes()
        .position(|b| b == b':')
        .unwrap_or_else(|| panic!("line was:: {}", line));
      let name_here = &strip_pub_const[..stop];
      //eprintln!("{}", name_here);
      if KEEPERS.contains(&name_here) {
        print!("{}", comments);
        println!("{}", line);
        println!();
      }
